...
var data;
console.log("hiii");
showData.open('GET', 'http://localhost:8000/carriers/?format=json', false);
...
function update() {
    ...
    var showData1 = new XMLHttpRequest();
        ...
        if(strUser==="ALL")
            showData1.open('GET', 'http://localhost:8000/carriers/?format=json', false);
        else
            showData1.open('GET', 'http://localhost:8000/carriers/?airport='+strUser+'&format=json', false);
        showData1.send();
        ...
        for (k in data1) {
            ...
            table_body += '<tr>';
            table_body += '<td>';
            table_body += data1[k]["name"] + '</br>';
            table_body += '</td>';

            table_body += '<td>';
            table_body += data1[k]["code"];
            table_body += '</td>';
